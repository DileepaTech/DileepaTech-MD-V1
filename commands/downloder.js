...
cmd({
        ...
        async(Void, citel, text) => {
            if (!text) return citel.reply('කරුණාකර මට ශ්‍රව්‍ය බවට වෙනස් කිරීමට වාක්‍ය ලබා දෙන්න.')
            let texttts = text
            citel.react("📢");
...
cmd({
        ...
        async(Void, citel, text) => {
            if (!text) return reply("ඔබ සොයන්නේ කුමන පින්තූරයක්ද?") && Void.sendMessage(citel.chat, {
                react: {
                    text: '❌',
                    key: citel.key
                }
    ...
    async(Void, citel, text) => {
        ...
        try {
            ...
            if (fileSizeInMegabytes <= dlsize) {
                let yts = require("secktor-pack");
            citel.reply(` *Download & Upload Your song* `);
                let search = await yts(text);
            citel.react("✅");
                let buttonMessage = {
...
const tsong = await fetchJson(`https://darkalphaxteam-api.cyclic.app/api/download/ytmp4?url=${anu.url}&apikey=prabath`)

const videolink = tsong.download

            citel.reply (`*Your Video Download & Uploade Please Wait...*`);
...
cmd({
    ...
    async(Void, citel, text) => {
        ...
        try {
            ...
            citel.reply(` *Download & Upload Your DocVideo* `);
